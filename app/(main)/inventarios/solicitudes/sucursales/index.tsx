...
import { getSolicitudesTraspaso, SolicitudTraspaso } from '@/services/api';
...
type FilterType = 'TODOS' | 'PENDIENTES' | 'SURTIDAS' | 'CANCELADAS';

const getEstatusDisplay = (estatus: string) => {
  switch (estatus) {
    case 'P': return { label: 'PENDIENTE', color: '#F59E0B' };
    case 'S': return { label: 'SURTIDA', color: '#10B981' };
    case 'C': return { label: 'CANCELADA', color: '#EF4444' };
    default: return { label: estatus, color: '#6B7280' };
  }
};
...
export default function SolicitudesListScreen() {
  ...
  const [solicitudes, setSolicitudes] = useState<SolicitudTraspaso[]>([]);
  ...
  const loadSolicitudes = useCallback(async () => {
    setLoading(true);
    try {
      const result = await getSolicitudesTraspaso(100);
      if (result.ok) {
  ...
  const filteredItems = useMemo(() => {
    return solicitudes.filter(item => {
      const matchesSearch = item.folio.toLowerCase().includes(searchQuery.toLowerCase()) || 
                           item.sucursal_origen.toLowerCase().includes(searchQuery.toLowerCase()) ||
                           item.sucursal_destino.toLowerCase().includes(searchQuery.toLowerCase());
      
      let matchesFilter = true;
      if (activeFilter === 'PENDIENTES') matchesFilter = item.estatus === 'P';
      if (activeFilter === 'SURTIDAS') matchesFilter = item.estatus === 'S';
      if (activeFilter === 'CANCELADAS') matchesFilter = item.estatus === 'C';

      // Filtro de fechas
      let matchesDate = true;
      if (startDate || endDate) {
        const itemDate = new Date(item.fecha);
        if (startDate) matchesDate = itemDate >= new Date(startDate);
        if (endDate && matchesDate) matchesDate = itemDate <= new Date(endDate + 'T23:59:59');
      }

      return matchesSearch && matchesFilter && matchesDate;
    });
  }, [solicitudes, searchQuery, activeFilter, startDate, endDate]);
  ...
  const renderItem = ({ item }: { item: SolicitudTraspaso }) => {
    const estatusInfo = getEstatusDisplay(item.estatus);
    
    return (
      <TouchableOpacity 
        activeOpacity={0.8}
        onPress={() => router.push({ pathname: '/(main)/inventarios/solicitudes/sucursales/detalle', params: { id: item.id.toString() } })}
        style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}
      >
        <View style={styles.cardTopRow}>
          <View style={styles.statusIndicator}>
            <View style={[styles.statusDot, { backgroundColor: estatusInfo.color }]} />
            <Text style={[styles.statusLabel, { color: colors.textTertiary }]}>
              {estatusInfo.label}
            </Text>
          </View>
          <Text style={[styles.timeText, { color: colors.textTertiary }]}>
              {formatTime(item.fecha_creacion)}
          </Text>
        </View>
        ...
        <View style={styles.routeRow}>
          ...
          <Text style={[styles.routeText, { color: colors.text }]} numberOfLines={1}>{item.sucursal_origen}</Text>
          <Ionicons name="arrow-forward" size={12} color={colors.textTertiary} style={{ marginHorizontal: 6 }} />
          <Ionicons name="location-outline" size={14} color={colors.textTertiary} />
          <Text style={[styles.routeText, { color: colors.textSecondary }]} numberOfLines={1}>{item.sucursal_destino}</Text>
        </View>

        <View style={[styles.cardFooter, { backgroundColor: colors.background + '50' }]}>
          <View style={styles.footerStat}>
            <Text style={[styles.footerStatLabel, { color: colors.textTertiary }]}>SKUs</Text>
          ...
          <View style={styles.footerStat}>
            <Text style={[styles.footerStatLabel, { color: colors.textTertiary }]}>UNIDADES</Text>
            <Text style={[styles.footerStatValue, { color: colors.text }]}>{item.total_unidades}</Text>
          </View>

          <View style={[styles.chevronContainer, { backgroundColor: '#FB923C' + '15' }]}>
  ...
  const FilterChip = ({ type, label }: { type: FilterType, label: string }) => (
    <TouchableOpacity 
      ...
      style={[
        styles.chip, 
        activeFilter === type ? { backgroundColor: '#FB923C' } : { backgroundColor: colors.surface, borderColor: colors.border, borderWidth: 1 }
      ]}
    >
      <Text style={[
        styles.chipText, 
      ...
      <View style={styles.headerSection}>
        <View style={styles.titleRow}>
          ...
          <View style={styles.headerActions}>
            <TouchableOpacity 
              onPress={() => router.push('/(main)/inventarios/solicitudes/sucursales/crear')}
            ...
            <TouchableOpacity 
              ...
              style={[
                ...
                { backgroundColor: (startDate || endDate) ? '#FB923C' : colors.surface }
              ]}
            >
        ...
        <View style={styles.filtersContainer}>
          ...
          <FilterChip type="SURTIDAS" label="SURTIDAS" />
        </View>
      </View>
          ...
          <View style={[styles.dateModalContent, { backgroundColor: colors.surface }]}>
            ...
            {!isCustomRange ? (
              <View style={styles.dateOptions}>
                ...
                >
                  <Ionicons name="today-outline" size={20} color="#FB923C" />
                ...
                >
                  <Ionicons name="calendar-number-outline" size={20} color="#FB923C" />
                ...
                <TouchableOpacity 
                  style={[styles.dateOption, { borderColor: colors.border, backgroundColor: '#FB923C' + '10' }]} 
                  onPress={() => setIsCustomRange(true)}
                >
                  <Ionicons name="options-outline" size={20} color="#FB923C" />
                  <Text style={[styles.dateOptionText, { color: '#FB923C' }]}>Rango Personalizado...</Text>
                </TouchableOpacity>
            ...
            {!isCustomRange && (
              <TouchableOpacity 
                style={[styles.dateModalClose, { backgroundColor: '#FB923C' }]}
                onPress={() => setShowDatePicker(false)}
              >
