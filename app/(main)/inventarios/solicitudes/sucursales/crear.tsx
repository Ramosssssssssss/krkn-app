...
export default function CrearSolicitudSucursalScreen() {
  ...
  const [successResult, setSuccessResult] = useState<{ folio: string; solicitud_id: number; inserted: number } | null>(null);
  ...
  const [destinoSucursalId, setDestinoSucursalId] = useState<number | null>(null);
  ...
  const destinoSucursalNombre = sucursales.find(s => s.id === destinoSucursalId)?.nombre || '';
  const destinoAlmacenNombre = almacenes.find(a => a.id === destinoAlmacenId)?.nombre || '';
  ...
  if (configError) {
    return (
      <View style={[styles.center, { backgroundColor: colors.background }]}>
        ...
        <Text style={{ marginTop: 12, color: colors.textSecondary }}>{configError}</Text>
        <TouchableOpacity style={styles.retryBtn} onPress={refreshConfig}>
          <Text style={{ color: '#FB923C', fontWeight: '700' }}>Reintentar</Text>
        </TouchableOpacity>
      </View>
    );
  }
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={styles.header}>
        <View style={styles.selectorsRow}>
          <TouchableOpacity 
            style={[styles.selector, { backgroundColor: colors.surface, borderColor: origenId ? '#FB923C' : colors.border }]}
            onPress={() => setShowOrigenModal(true)}
          >
            <Text style={[styles.selectorLabel, { color: colors.textSecondary }]}>SOLICITA (Origen)</Text>
          ...
          <TouchableOpacity 
            style={[styles.selector, { backgroundColor: colors.surface, borderColor: destinoAlmacenId ? '#FB923C' : colors.border }]}
            onPress={() => setShowDestinoModal(true)}
          >
            <Text style={[styles.selectorLabel, { color: colors.textSecondary }]}>A (Destino)</Text>
            <Text style={[styles.selectorValue, { color: colors.text }]} numberOfLines={1}>
              {destinoAlmacenId ? `${destinoSucursalNombre} (${destinoAlmacenNombre})` : 'Seleccionar...'}
            </Text>
          </TouchableOpacity>
        </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyState}>
            ...
            <Text style={[styles.emptyText, { color: colors.textTertiary }]}>No hay artículos escaneados</Text>
          </View>
        }
      />
      ...
      <View style={[styles.footer, { paddingBottom: insets.bottom + 16, backgroundColor: colors.surface }]}>
        <View style={styles.summary}>
          <View>
            <Text style={[styles.summaryLabel, { color: colors.textSecondary }]}>SKUs</Text>
            <Text style={[styles.summaryValue, { color: colors.text }]}>{totalArticles}</Text>
          </View>
          <View style={styles.summaryDivider} />
          <View>
            <Text style={[styles.summaryLabel, { color: colors.textSecondary }]}>UDs</Text>
            <Text style={[styles.summaryValue, { color: colors.text }]}>{totalUnits}</Text>
          </View>
        </View>

        <TouchableOpacity 
          style={[styles.btnSubmit, { backgroundColor: '#FB923C', opacity: (isSubmitting || articles.length === 0) ? 0.7 : 1 }]} 
          onPress={handleSave}
      ...
      <Modal visible={showOrigenModal} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: colors.surface }]}>
            <Text style={[styles.modalTitle, { color: colors.text }]}>¿Quién solicita?</Text>
            <ScrollView style={{ maxHeight: 400 }}>
              {sucursales.map((s) => (
                <TouchableOpacity 
                  key={s.id} 
                  style={[styles.modalItem, { borderBottomColor: colors.border }]}
                  onPress={() => { setOrigenId(s.id); setShowOrigenModal(false); }}
                >
                  <Text style={[styles.modalItemText, { color: colors.text }]}>{s.nombre}</Text>
                </TouchableOpacity>
              ))}
            </ScrollView>
            <TouchableOpacity style={styles.modalClose} onPress={() => setShowOrigenModal(false)}>
              <Text style={{ color: '#FB923C', fontWeight: '800' }}>CERRAR</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
      ...
      <Modal visible={showDestinoModal} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: colors.surface }]}>
            <Text style={[styles.modalTitle, { color: colors.text }]}>¿A quién se solicita?</Text>
            <ScrollView style={{ maxHeight: 400 }}>
              {almacenes.map((a) => {
                const sucNombre = sucursales.find(s => s.id === a.sucursalId)?.nombre || '';
                return (
                  <TouchableOpacity 
                  ...
                  >
                    <View>
                      <Text style={[styles.modalItemText, { color: colors.text }]}>{sucNombre}</Text>
                      <Text style={{ fontSize: 12, color: colors.textSecondary }}>{a.nombre}</Text>
                    </View>
                  </TouchableOpacity>
                );
              })}
            </ScrollView>
            <TouchableOpacity style={styles.modalClose} onPress={() => setShowDestinoModal(false)}>
              <Text style={{ color: '#FB923C', fontWeight: '800' }}>CERRAR</Text>
            </TouchableOpacity>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  selector: { flex: 1, padding: 12, borderRadius: 12, borderWidth: 1, justifyContent: 'center' },
  ...
  footer: { padding: 16, borderTopWidth: 1, borderTopColor: 'rgba(0,0,0,0.05)', flexDirection: 'row', alignItems: 'center', gap: 16 },
  ...
  btnSubmit: { flex: 1, height: 56, borderRadius: 28, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 12, elevation: 4 },
