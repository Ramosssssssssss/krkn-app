...
import { DoctoEntrada, getDoctosEntradas } from '@/services/inventarios';
...
export default function EntradasListScreen() {
  ...
  const [entradas, setEntradas] = useState<DoctoEntrada[]>([]);
  ...
  // Estado para selección manual (Año-Mes-Día) - Ya no se usa para UI pero se mantiene si es necesario para lógica
  ...
  const loadEntradas = useCallback(async () => {
    setLoading(true);
    try {
      const data = await getDoctosEntradas({
        limit: 100,
  ...
  useEffect(() => {
    loadEntradas();
  }, [loadEntradas, startDate, endDate]);
  ...
  }, [entradas, searchQuery, activeFilter]);
  ...
  const renderItem = ({ item }: { item: DoctoEntrada }) => (
    ...
    >
      ...
      <View style={[styles.cardFooter, { backgroundColor: colors.background + '50' }]}>
        ...
        <View style={[styles.chevronContainer, { backgroundColor: colors.accent + '15' }]}>
