...
export default function CompanyCodeScreen() {
  ...
  const { setCompanyCode: saveCompanyCode, login, selectDatabase } = useAuth();
  ...
  const validateCompanyCode = async () => {
    ...
    if (code === "DIEGO") {
      setIsLoading(true);
      try {
        const mockCompany = { EMPRESA_ID: 1, CODIGO: "DIEGO" };
        const mockDatabases = [
          {
            id: 1,
            nombre: "Base de Datos Diego",
        ...
        saveCompanyCode("DIEGO", mockCompany as any, mockDatabases as any);
        selectDatabase(mockDatabases[0] as any);
        await login(mockUser as any, "diego-bypass-token", {});

        router.replace("/(main)");
        return;
      } catch (err) {
        console.error("Error in DIEGO bypass:", err);
        setError("Error al iniciar sesión bypass");
      } finally {
        setIsLoading(false);
      }
      return;
    ...
    try {
      ...
      const response = await fetch(
        `https://app.krkn.mx/api/get-databases.php?companyCode=${encodeURIComponent(code)}`,
        {
          method: "GET",
        },
      );
          ...
          >
            ...
            >
              <TextInput
                ...
                value={companyCode}
                onChangeText={(text) => {
                  setCompanyCode(text.toUpperCase().replace(/[^A-Z0-9-]/g, ""));
                  setError("");
                }}
              ...
              />
              <Text
                style={[styles.inputSuffix, { color: colors.textTertiary }]}
              >
                .krkn.mx
              </Text>
            </View>
            ...
            <TouchableOpacity
              style={[
                styles.btn,
                (!companyCode.trim() || isLoading) && styles.btnDisabled,
              ]}
                ...
                {isLoading ? (
                  ...
                  <>
                    ...
                    <Ionicons
                      name="arrow-forward"
                      size={18}
                      color={!companyCode.trim() ? colors.textTertiary : "#fff"}
                    />
                  </>
                )}
              </LinearGradient>
            ...
            >
              <Text style={[styles.helpText, { color: colors.textSecondary }]}>
                ¿No tienes código?{" "}
                <Text style={{ color: colors.accent }}>Contáctanos</Text>
              </Text>
            </TouchableOpacity>
          </Animated.View>
