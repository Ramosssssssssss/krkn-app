...
export default function LocationChip({ sucursalNombre, almacenNombre, ubicacion, onPress }: Props) {
  const colors = useThemeColors();

  return (
    <TouchableOpacity
      style={[st.chip, { backgroundColor: colors.surface, borderColor: colors.border }]}
      onPress={onPress}
    >
      ...
      <View style={st.textBox}>
        ...
        <Text style={[st.ubicacion, { color: colors.text }]} numberOfLines={2}>
          {ubicacion ? (
            <>
              Ubicación:{" "}
              <Text style={{ color: colors.accent, fontWeight: "700" }}>{ubicacion}</Text>
            </>
          ) : (
...
const st = StyleSheet.create({
  ...
  label: { fontSize: 11, fontWeight: "400", textTransform: "uppercase", letterSpacing: 0.5 },
